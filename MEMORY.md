...
5. **Design tokens**
   ...
   - **LogoMark single source**: `src/brand/logoMark.ts` owns the 40×40 geometry (`LOGO_DOT`, `LOGO_ARC_INNER/OUTER`), the per-surface palette (`brand` indigo/orange, `cafe` terracotta/brass), and `buildLogoMarkSvg({stroke, padding, surface, responsive})`. `LogoMark.jsx` renders it (stroke 3.2, `responsive: true`); `scripts/pwaIconsPlugin.mjs` parses the same module (regex extraction — no TS loader) and generates ALL of `public/favicon*.svg|*.png|*.ico` from it (favicon stroke 5.5, padding 1.5; rasters/maskables as before). Everything in `public/` except `manifest.webmanifest` is generated + gitignored; edits to `logoMark.ts` regenerate on next dev/build (mtime-stamped, parity-tested in `scripts/pwaIconsPlugin.test.ts`).
   - **Favicons & browser chrome** are swapped per surface by `src/components/FaviconManager` (mounted in `App.jsx` inside `BrowserRouter`, same `isDemoCafePath` set): product = indigo `favicon.svg` / `favicon-16|32.png` / `favicon.ico`, theme-color `#faf9f7`; café = terracotta `favicon-demo.*`, theme-color `#f3eadc`. `index.html` declares every slot (SVG + PNG 32/16 + ICO + `apple-touch-icon` + `manifest.webmanifest` + `theme-color`); the manifest ships 192/512 `any`+`maskable` icons; no café manifest variant (install identity stays Omnitaps). Bump `?v=` in BOTH `index.html` and `FaviconManager.tsx` together when art changes. `index.html` declares every slot: SVG favicon + PNG 32/16 + `shortcut icon` ICO + `apple-touch-icon` 180×180 + `manifest.webmanifest` + `theme-color`. The manifest ships 192/512 icons in `any` + `maskable` purposes (porcelain background; maskable art inside the 80% safe zone) for home-screen install; there is **no café manifest variant** — install identity stays Omnitaps, only tab favicon/theme-color swap per surface. Bump the `?v=` in BOTH `index.html` and `FaviconManager.tsx` together when art changes.
